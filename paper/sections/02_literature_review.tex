...
% STATUS: both previously-open citation flags (7x7 fuzzy partition
% precedent; SMC sliding-surface precedent) are now resolved with real,
% Crossref-verified citations (fapi2025fuzzy, ahmad2020application) --
% the three unresolved "candidate" SMC citations from the earlier search
% (zhang2022novel, mostafa2020tracking, contreras2024novel) were removed
% from references.bib since they were never actually a good match and are
% now superseded. references.bib has 29 total citations, 15 of which are
% 2023-2025 -- clears both publication_supplement.md checklist items
% (>=20 total, >=15 recent). Every citation has been verified against
% Crossref metadata (see references.bib comments) -- do not add a citation
% here without the same verification.
...
fuzzy-MPPT literature \cite{fapi2025fuzzy}. \cite{sunkara2024novel} presents a hybrid fuzzy
...
$s = dP/dV$ \cite{ahmad2020application}, driving the system toward the MPP condition) and a reaching
...
(\texttt{config.py}) could be extended toward. Per CLAUDE.md's own caution,
no single PV-SMC paper combines this exact surface, reaching law, and
boundary-layer choice at once (a search for one during this literature
review turned up several partial matches -- e.g.\ a multi-power reaching
law with a sigmoid switching function -- but none matching all three
design choices simultaneously); the design lineage is instead cited
piece-by-piece: $s=dP/dV$ as the common surface choice
\cite{ahmad2020application}, the exponential reaching law from
\cite{gao1993variable}, and the boundary-layer chattering mitigation from
\cite{slotine1991applied}.
...
% This section's citation-count and citation-flag TODOs are now resolved;
% remaining work before submission is a final proofread pass, not more
% citations.

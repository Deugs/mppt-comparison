...
from a formula of our own design (not adapted from a specific prior
publication), chosen to reproduce a conventional error/change-of-error
$\to$ duty-step rule table entry-for-entry while remaining parameterizable
to the 5- and 3-label reduced bases used later in this section:
\begin{equation}
...
empirically-grounded scale used for the fuzzy controller's $E$ input. This
discretization is coarse by deliberate choice, not oversight: it keeps
tabular $Q$-learning tractable to train (Section~\ref{sec:conclusion}
returns to whether finer discretization or function approximation would
narrow the held-out generalization gap reported in
Section~\ref{sec:results}, a question this paper's fair-comparison design
does not attempt to answer with the same tabular architecture). The
...
% Resolved: per CLAUDE.md's own caution, no single PV-SMC paper combines
% this exact surface/reaching-law/boundary-layer combination at once, so
% the design lineage is cited piece-by-piece instead -- see
% 02_literature_review.tex's Model-Based Nonlinear Control subsection
% (ahmad2020application for the s=dP/dV surface choice, gao1993variable
% for the reaching law, slotine1991applied for the boundary layer).

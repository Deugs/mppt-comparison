...
- [x] Validate simulated I-V/P-V curves against datasheet (RMSE < 2%) — STC: all four key points within 2% (max error 0.068%, at Pmax), `tests/test_pv_model.py`. NOCT (G=800 W/m^2, Tc=45°C): RMSE across the 5 key points is 1.35% (`test_noct_validation_within_2_percent_rmse`), under the 2% criterion, though the individual Pmax point alone is 2.44%. Low irradiance (200 W/m^2): the datasheet only gives a coarse "+95.5% efficiency retention from 1000→200 W/m^2" claim, not a full I-V table, so this validation is a single retention-ratio check rather than a point match — model measures 93.5% retention, ~2pp below the claim (`test_low_irradiance_efficiency_retention_is_reasonably_close`), plausibly because this simplified model holds Rsh irradiance-independent rather than scaling it as a full five-parameter model would. **Found and fixed a real bug while closing this out**: `TwoDiodeModel`'s temperature dependence previously came only through the thermal-voltage term, leaving Iph/Is temperature-independent — this made Voc *increase* with temperature (backwards; datasheet Kv=-0.34%/°C says it should decrease). Fixed via the standard De Soto et al. (2006) Iph(G,T)/Is(T) extension in `pv_model.py` (`photocurrent()`, `_saturation_current()`), which is identically 1 at T=T_ref so STC behavior is provably unchanged (confirmed: all STC tests pass with no modification). `simulate.py`'s brentq search-bound estimates for `pv_operating_point()`/`pv_string_operating_point()` were also updated to use the temperature-aware `photocurrent()` rather than an STC-only estimate, since the old 1% margin could theoretically under-bracket the root at high temperature given Ki>0. Regenerated the one scenario this affects (`temperature_step` — every other scenario runs at constant 25°C) for all 7 algorithms; surfaced a real, previously-hidden finding in the process — see Phase 4 notes below.
...
- [x] Implement IncCond with boundary condition handling (`src/algorithms/inc_cond.py`; a real bug was found and fixed here during the Phase 4 paper's peer-review audit — see the "Note on IncCond/SMC duty-cycle-clamp bug" below)
...
- [x] Sensitivity analysis: test 5x5 and 3x3 fuzzy rule bases — reduced rule bases derived (same generating formula, fewer labels) and verified to find the STC MPP in `tests/test_fuzzy_logic.py`; the full performance-tradeoff comparison was still missing after Phase 3 completed (found while scoping Phase 4 — `results/comparison_table.csv` had 0 `fuzzy_5x5`/`fuzzy_3x3` rows despite this being flagged a "publishable contribution"). Closed in Phase 4 prep: `scenarios.build_fuzzy_rule_base_variants()` + `python -m src.scenarios --fuzzy-variants-only` ran both variants through the full 50-run x 11-scenario sweep and appended their rows. Result at `results/fuzzy_sensitivity_table.csv`: at steady state, fuzzy_5x5 (99.88% tracking efficiency) is very close to the full 7x7 fuzzy_logic (99.87%) — real support for the "5x5 matches 7x7" contribution CLAUDE.md hoped for; fuzzy_3x3 (99.01%) is measurably lower but still close. See `src/plotting.py`'s `plot_fuzzy_sensitivity()` figure for the full per-scenario comparison. **Correction (caught once `fuzzy_sensitivity_stats()` added real hypothesis tests instead of eyeballing overlapping CIs):** 7x7 vs. 5x5 is not actually "statistically indistinguishable" — with n=50 paired samples the tiny 0.01-percentage-point gap reaches significance (Holm-corrected paired t-test, $p=1.8\times10^{-4}$), just with a medium effect size (Cohen's d=-0.57) rather than the large effect separating either from 3x3 (d=0.66 both ways). The accurate claim is "a real but practically negligible difference," not "indistinguishable" — see `paper/sections/07_results.tex`'s corrected Fuzzy Rule-Base Sensitivity paragraph and `results/fuzzy_sensitivity_stats.csv`.
...
**Note on IncCond/SMC duty-cycle-clamp bug (found during the Phase 4 paper's peer-review audit, fixed in Phase 2 code):** direct trajectory tracing of the `multi_level_irradiance` scenario found both `IncrementalConductance` and `SlidingModeControl` permanently freeze at `MPPT_DUTY_MIN` and never recover, even once irradiance later rises enough that the true MPP becomes reachable again. Root cause: a boost converter can only present `R_in = R_load*(1-D)^2 <= R_load`, so at low irradiance (verified at 200 W/m2: true MPP needs R_in=18.6 Ohm, but R_load=9.2 Ohm) the true MPP is unreachable at *any* valid duty cycle, driving duty to the clamp. Once there, `|dV|~=0` between samples — indistinguishable from genuinely being at the MPP for both algorithms' boundary-case logic (IncCond's `dV~=0` branch; SMC's `s=dP/dV` reading). P&O never has this problem because it has no "hold" state at all — every step perturbs, so it keeps dithering at the boundary until escape is discovered. Fixed in both `src/algorithms/inc_cond.py` and `src/algorithms/sliding_mode.py` by overriding a "hold" (or a correction that would push further into a limit already reached) with a probe step away from that limit, using each algorithm's own existing step/correction scale — the only way to tell "converged" apart from "stuck" is to actually try moving. Verified empirically (direct trajectory re-trace) and via two new regression tests (`test_recovers_from_duty_clamp_when_mpp_later_becomes_reachable` in `tests/test_inc_cond.py` and `tests/test_sliding_mode.py`). All pre-existing STC/chattering tests for both algorithms still pass unmodified. Monte Carlo data for both algorithms was regenerated across all 11 scenarios afterward (`results/comparison_table.csv`); see `paper/sections/07_results.tex` for the updated numbers.
...
- [x] Statistical analysis: ANOVA / t-test between algorithms — `src/analysis.py`: `run_anova()` (one-way ANOVA, `scipy.stats.f_oneway`) and `run_pairwise_ttests()` (paired t-test, `scipy.stats.ttest_rel`, paired by `run_id` per the identical-seed Monte Carlo design) over every `(scenario, metric)` combination, restricted by default to the 5 core algorithms (`CORE_ALGORITHMS`) and excluding `_computational_burden` (n=1, not a distribution). Output: `results/statistical_tests.csv` (924 rows: 77 ANOVA + 770 paired t-test + 77 Friedman — see below). Both explicitly report "insufficient data"/0 usable pairs rather than crashing or silently computing on too-small or all-NaN groups. Note for whoever reads this table next: a handful of near-constant metric groups (e.g. `sensor_noise_robustness`/`oscillation_p2p_w` for algorithms that lock onto the same value every run) trigger scipy `ConstantInputWarning`/precision-loss warnings — not a bug, just means the F/t statistic is close to undefined for that group; check `n_valid` and `std` in `summary_table.csv` before trusting a very large/small p-value in that row.
- [x] Deepen the statistical rigor beyond raw p-values (added after Phase 4 was first marked complete, prompted by "how do we make this richer" scoping): (1) **effect sizes** — `run_anova()` now also reports eta-squared (`_eta_squared()`, computed directly from group SS, not derived from F) and `run_pairwise_ttests()` reports paired Cohen's d (`_cohens_d_paired()` = mean(diff)/std(diff)); a p-value alone doesn't say whether hundreds of tests found anything *large*, and reviewers ask. (2) **Holm-Bonferroni correction** (`holm_bonferroni()`, hand-implemented — no new dependency beyond the locked `scipy`/`numpy`/`pandas` stack) applied within each `(scenario, metric)`'s family of ~10 pairwise comparisons (the natural post-hoc family following one ANOVA), added as a `p_holm` column alongside the raw `p_value` in `build_full_ttest_table()`. (3) **Friedman test** (`run_friedman_test()`/`build_full_friedman_table()`, `scipy.stats.friedmanchisquare`) as a rank-based repeated-measures companion to the one-way ANOVA — the one-way ANOVA treats the 5 algorithm groups as independent samples and so ignores the identical-seed run-id pairing that `run_pairwise_ttests()` already exploits; Friedman uses that pairing directly and doesn't need the F-test's normality assumption. Verified real, correctly-attributed citations added for all three (`cohen1988statistical`, `holm1979simple`, `friedman1937use` in `references.bib`, flagged as web-search-verified rather than Crossref-DOI-verified like the "SETTLED" block, since Holm 1979 predates DOIs and Cohen 1988 is a book) and cited at first use in `07_results.tex`'s steady-state paragraph, which now also reports real eta-squared/Friedman/Cohen's-d numbers for the tracking-efficiency and oscillation ANOVAs alongside the pre-existing F/t/p numbers (all cross-checked to match after regenerating `results/statistical_tests.csv` — the raw F/t/p values were unchanged, confirming the new columns are additive, not a silent recomputation). 13 new tests in `tests/test_analysis.py` (144/144 total passing), including the two genuinely-different-from-nan-for-a-reason edge cases: `_eta_squared()` on identical groups (0, not NaN — real "no effect" answer) vs. on every value being the *same constant* (NaN — ss_total=0 makes the ratio literally undefined), and Friedman's own degenerate all-tied-block case (documented in `src/analysis.py`'s module docstring rather than silently producing a misleading number).
- [x] Extend the same treatment to the paper's other results subsections, not just steady-state (follow-up to the above, same session): (1) **Partial shading** (`07_results.tex`'s "Partial Shading" subsection) — the three fully-zero-convergence scenarios are correctly left untested (degenerate, every group constant at 0%, explicitly called out as such rather than silently omitted), but Pattern C (the one scenario with real cross-algorithm variance) now reports a real ANOVA ($F=39.11$, $\eta^2=0.390$), Friedman ($\chi^2=175.15$), and Holm-corrected paired-t-test/Cohen's-d numbers confirming IncCond's 44% convergence reflects a real, large-effect-size ($d\approx-1.1$ vs. every other algorithm) tracking-efficiency deficit, not noise from a few borderline runs. (2) **Fuzzy rule-base sensitivity** — added `fuzzy_sensitivity_stats()` (same ANOVA/paired-t-test/Friedman treatment as the core algorithms, restricted to `FUZZY_VARIANT_ALGORITHMS` via the `algorithms=` parameter both `build_full_*` functions already supported) and a new `results/fuzzy_sensitivity_stats.csv` output. This caught a real inaccuracy in the *previous* draft's claim: "7x7 vs. 5x5 are statistically indistinguishable" was based on eyeballing overlapping 95% CIs, but a real paired t-test shows they *are* significantly different ($p_{\text{Holm}}=1.8\times10^{-4}$) — with n=50 paired samples, even the tiny 0.01-percentage-point gap reaches significance. The effect size clarifies what CI-overlap couldn't: Cohen's d=-0.57 (medium) for 7x7-vs-5x5 versus d=0.66 (also medium, slightly larger) separating 3x3 from either — so the accurate claim is "a real but practically negligible difference," corrected in both `07_results.tex` and this file's Phase 2 note above. (3) **Computational burden** deliberately does *not* get this treatment — `EXCLUDED_SCENARIOS` already excludes `_computational_burden` from all of ANOVA/t-test/Friedman because it's a single fixed-operating-point measurement per algorithm (n=1), not a Monte Carlo distribution; there's nothing to test. (4) Q-learning train-vs-held-out is a different comparison shape entirely (one algorithm's own two condition-tagged subsets, not paired by run_id across algorithms) that none of the existing test functions compute — flagged here as a real gap for whoever picks this up next, not silently left inconsistent with the rest of the paper's now-uniform statistical treatment.
...
- [x] Write literature review section — `paper/sections/02_literature_review.tex` organized by the same four paradigms used throughout the paper. All citations (29 total, 15 from 2023-2025) independently verified against Crossref metadata (title/authors/journal/volume/pages/DOI all cross-checked; caught two errors along the way — Utkin's real title is "...Sliding *Modes*", not "...Sliding Mode Control", and a bibliographic-text Crossref query once returned an unrelated peer-review-report record instead of the actual article, caught by checking the metadata didn't look like a real article and re-verified via direct DOI lookup). Both previously-open Citation Flags are now resolved with real citations: the 7x7 fuzzy partition precedent (`fapi2025fuzzy`) and the SMC sliding-surface precedent (`ahmad2020application`, cited only for the $s=dP/dV$ surface choice — CLAUDE.md's caution that no single paper combines this project's exact surface+reaching-law+boundary-layer combination still holds, so that design lineage stays cited piece-by-piece: surface/reaching-law/boundary-layer each from their own source). Three earlier "candidate" SMC citations (`zhang2022novel`, `mostafa2020tracking`, `contreras2024novel`) were removed from `references.bib` once superseded — they were never actually cited in live text, only inside `%`-comments, which a citation-consistency check must explicitly account for (a naive regex that doesn't skip comment lines will falsely count a commented-out `\cite{}` as live). `publication_supplement.md`'s checklist (>=20 references, >=15 from 2023-2025) is now met.
...
**Phase 4 analysis/figures status: complete.** `pytest tests/ -v --cov=src` at 129/129 (4 added for the PV-model temperature-physics fix below). Headline findings the statistical/figure pipeline surfaced, worth carrying into the paper's Results & Discussion:
...
- **Fuzzy rule-base reduction genuinely works, in the practically-relevant sense**: 5x5 (99.88%) vs.\ 7x7 (99.87%) tracking efficiency at steady state is a real, statistically-significant difference at n=50 (not "indistinguishable" — see the correction in the Phase 2 sensitivity-analysis note above), but a small/medium one (Cohen's d=-0.57) versus the larger effect separating either from 3x3 (d=0.66) — the accurate framing is "a real but practically negligible difference," which is what actually matters for the "5x5 preserves 7x7's performance" claim.
...
- **Q-learning's generalization gap isn't just about partial shading.** After the temperature-physics fix above, `temperature_step` (previously showing a misleadingly-okay ~91% tracking efficiency because of the temperature-model bug's side effects on voltage-bin discretization) now shows Q-learning at 64.3% — comparable in severity to 2 of the 4 partial-shading patterns, driven by sustained large-amplitude oscillation after the 25→50°C step, not by a convergence-rate change (still 80%, unchanged). A single-dimension held-out condition (temperature, same topology) is enough to trigger a severe failure, not only a full topology change like partial shading — see `paper/sections/07_results.tex`'s Q-learning subsection.

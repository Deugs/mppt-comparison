...
def _relative_mpp_error(agent, pv_model, irradiance, iterations=200):
    agent.reset()
    history = run_tracking(agent, pv_model, iterations=iterations, irradiance=irradiance)
...
def test_finds_mpp_at_stc(pv_model_stc, trained_agent):
    assert _relative_mpp_error(trained_agent, pv_model_stc, config.STC_IRRADIANCE) < config.MPPT_CONVERGENCE_THRESHOLD
...
def test_finds_mpp_on_training_irradiances(pv_model_stc, trained_agent, irradiance):
    assert _relative_mpp_error(trained_agent, pv_model_stc, irradiance) < config.MPPT_CONVERGENCE_THRESHOLD
...
def test_generalizes_to_held_out_irradiance(pv_model_stc, trained_agent):
    ...
    assert _relative_mpp_error(trained_agent, pv_model_stc, held_out_irradiance) < config.MPPT_CONVERGENCE_THRESHOLD
...
def test_duty_cycle_stays_within_bounds(pv_model_stc, trained_agent):
    trained_agent.reset()
    history = run_tracking(trained_agent, pv_model_stc, iterations=200, irradiance=config.STC_IRRADIANCE)

...
def test_open_circuit_voltage_matches_datasheet(stc_params):
    ...
    model = TwoDiodeModel(stc_params, num_cells=config.PANEL_NS)
    voc_model = model.open_circuit_voltage(
...
def test_bypass_diode_clamps_heavily_shaded_group(stc_params):
    ...
    unshaded_isc = shaded_group.model.current_at_voltage(
        0.0, irradiance=config.STC_IRRADIANCE
    )
...
def test_module_voltage_is_sum_of_group_voltages(stc_params):
    ...
    v_single_group = module.groups[0].voltage_at_current(
        low_current, irradiance=config.STC_IRRADIANCE
    )
...
def test_voc_decreases_and_isc_increases_with_temperature(stc_params):
    ...
    voc_stc = model.open_circuit_voltage(
        config.STC_IRRADIANCE, config.STC_TEMPERATURE_C
    )
    ...
    isc_stc = model.current_at_voltage(
        0.0, config.STC_IRRADIANCE, config.STC_TEMPERATURE_C
    )

...
def test_run_scenario_produces_correctly_shaped_result(pv_model_stc):
    ...
    result = scenarios.run_scenario(scenario, algorithm, pv_model_stc, sample_period_s=0.01)
...
def test_reference_mpp_curve_caches_piecewise_constant_scenario(pv_model_stc):
    ...
    curve = scenarios.reference_mpp_curve(pv_model_stc, times, scenario.irradiance_fn, scenario.temperature_fn)
...
def test_run_scenario_reflects_irradiance_step_in_theoretical_max(pv_model_stc):
    ...
    assert result.theoretical_max_powers[idx_before_step] > result.theoretical_max_powers[idx_after_step]
...
def test_sensor_noise_perturbs_algorithm_decisions(pv_model_stc):
    ...
    quiet_result = scenarios.run_scenario(
        scenario, quiet_algorithm, pv_model_stc, sample_period_s=0.01, rng=random.Random(0)
    )
    ...
    noisy_result = scenarios.run_scenario(
        noisy_scenario, noisy_algorithm, pv_model_stc, sample_period_s=0.01, rng=random.Random(0)
    )
...
def test_run_monte_carlo_produces_requested_number_of_runs(pv_model_stc):
    scenario = scenarios.scenario_steady_state()
    algorithm = PerturbObserve()
    results = scenarios.run_monte_carlo(scenario, algorithm, pv_model_stc, num_runs=5, sample_period_s=0.02)
...
def test_run_monte_carlo_is_reproducible_given_same_seed(pv_model_stc):
    ...
    results_a = scenarios.run_monte_carlo(
        scenario, PerturbObserve(), pv_model_stc, num_runs=3, base_seed=42, sample_period_s=0.02
    )
...
def test_run_monte_carlo_uses_identical_seeds_across_algorithms(pv_model_stc):
    ...
    po_results = scenarios.run_monte_carlo(
        scenario, PerturbObserve(), pv_model_stc, num_runs=3, base_seed=7, sample_period_s=0.02
    )
...
def test_results_to_rows_has_one_row_per_run_per_metric(pv_model_stc):
    scenario = scenarios.scenario_steady_state()
    results = scenarios.run_monte_carlo(scenario, PerturbObserve(), pv_model_stc, num_runs=3, sample_period_s=0.02)
...
def test_computational_burden_rows_normalizes_to_baseline():
    ...
    burden_rows = {r["algorithm"]: r["value"] for r in rows if r["metric"] == "relative_burden"}
    assert burden_rows["p_and_o"] == pytest.approx(1.0)
    assert burden_rows["fuzzy_logic"] == pytest.approx(4.0)

    time_rows = {r["algorithm"]: r["value"] for r in rows if r["metric"] == "mean_step_time_s"}

...
def test_summarize_computes_correct_mean_std_ci_and_median_iqr():
    ...
    df = pd.DataFrame(_rows("p_and_o", "steady_state", "tracking_efficiency_pct", values))
    ...
    assert row["iqr"] == pytest.approx(np.percentile(values, 75) - np.percentile(values, 25))
...
def test_summarize_single_value_group_has_zero_std_and_nan_ci():
    df = pd.DataFrame(_rows("p_and_o", "_computational_burden", "mean_step_time_s", [0.001]))
...
def test_summarize_convergence_rate_reflects_partial_non_convergence():
    ...
    df = pd.DataFrame(_rows("inc_cond", "sensor_noise_robustness", "convergence_time_s", values))
    ...
    assert 0.0 <= rate_row["ci95_low"] <= rate_row["mean"] <= rate_row["ci95_high"] <= 1.0
...
def test_summarize_convergence_rate_is_explicit_zero_when_nothing_converges():
    ...
    values = [np.nan] * 50
    df = pd.DataFrame(_rows("p_and_o", "partial_shading_simple", "convergence_time_s", values))
    ...
    assert rate_row["ci95_high"] > 0.0  # Wilson CI is not a degenerate zero-width interval
...
def test_summarize_splits_q_learning_rows_by_ql_condition():
    df = pd.DataFrame(
        _rows("q_learning", "steady_state", "tracking_efficiency_pct", [99.0, 98.0], ql_condition="train")
        + _rows("q_learning", "temperature_step", "tracking_efficiency_pct", [80.0, 82.0], ql_condition="held_out")
    )
    result = analysis.summarize(df)
...
def test_run_anova_detects_a_real_group_difference():
    df = pd.DataFrame(
        _rows("p_and_o", "steady_state", "oscillation_p2p_w", [1.0, 1.1, 0.9, 1.05, 0.95])
        + _rows("inc_cond", "steady_state", "oscillation_p2p_w", [1.0, 0.9, 1.1, 0.95, 1.05])
        + _rows("sliding_mode", "steady_state", "oscillation_p2p_w", [5.0, 5.1, 4.9, 5.05, 4.95])
    )
    result = analysis.run_anova(df, "steady_state", "oscillation_p2p_w", algorithms=("p_and_o", "inc_cond", "sliding_mode"))
...
def test_run_anova_identical_groups_do_not_falsely_reject():
    ...
    result = analysis.run_anova(df, "steady_state", "oscillation_p2p_w", algorithms=("p_and_o", "inc_cond"))
    assert result["p_value"] > 0.05
...
def test_run_pairwise_ttests_pairs_by_run_id_and_detects_a_shift():
    df = pd.DataFrame(
        _rows("p_and_o", "steady_state", "tracking_efficiency_pct", [99.0, 98.5, 99.2, 98.8, 99.1])
        + _rows("sliding_mode", "steady_state", "tracking_efficiency_pct", [95.0, 94.5, 95.2, 94.8, 95.1])
    )
    results = analysis.run_pairwise_ttests(df, "steady_state", "tracking_efficiency_pct", algorithms=("p_and_o", "sliding_mode"))
...
def test_run_pairwise_ttests_drops_unpaired_nan_runs():
    df = pd.DataFrame(
        _rows("p_and_o", "sensor_noise_robustness", "convergence_time_s", [0.05, np.nan, 0.06, 0.055])
        + _rows("inc_cond", "sensor_noise_robustness", "convergence_time_s", [0.04, 0.045, np.nan, 0.05])
    )
    results = analysis.run_pairwise_ttests(df, "sensor_noise_robustness", "convergence_time_s", algorithms=("p_and_o", "inc_cond"))
    result = results[0]
...
def test_run_pairwise_ttests_reports_missing_algorithm():
    df = pd.DataFrame(_rows("p_and_o", "steady_state", "tracking_efficiency_pct", [99.0, 98.0]))
    results = analysis.run_pairwise_ttests(df, "steady_state", "tracking_efficiency_pct", algorithms=("p_and_o", "q_learning"))
    result = results[0]
...
def test_build_full_anova_table_excludes_computational_burden_scenario():
    df = pd.DataFrame(
        ...
        + _rows("inc_cond", "steady_state", "tracking_efficiency_pct", [99.1, 98.1, 99.4])
